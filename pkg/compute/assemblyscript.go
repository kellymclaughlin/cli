...
func (a AssemblyScript) Verify(out io.Writer) error {
	...
	//
	// A valid NPM package is needed for compilation and to assert wheether the
...
func (a AssemblyScript) Build(out io.Writer, verbose bool) error {
	...
	compiler := NewCompiler(cmd, args, []string{}, verbose, out)
	if err := compiler.Exec(); err != nil {
		return err
	}
